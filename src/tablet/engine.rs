...
impl LsmEngine {
    ...
    pub async fn do_compaction(&self) -> Result<bool> {
        ...
        // Compute GC parameters for MVCC compaction GC.
...
impl PessimisticStorage for LsmEngine {
    ...
    fn alloc_and_reserve_commit_lsn(&self, owner_start_ts: Timestamp) -> u64 {
        LsmEngine::alloc_and_reserve_commit_lsn(self, owner_start_ts)
    }
...
mod tests {
    ...
    async fn test_tablet_engine_smoke_open_write_read() {
        ...
        let engine = crate::tablet::lsm::TabletEngine::open(config).unwrap();

        let mut batch = new_batch(10);
        batch.put(b"tablet_smoke_key".to_vec(), b"tablet_smoke_value".to_vec());
        engine.write_batch(batch).unwrap();

        assert_eq!(
            get_for_test(&engine, b"tablet_smoke_key").await,
            Some(b"tablet_smoke_value".to_vec())
        );
    }

    #[tokio::test]
    async fn test_tablet_engine_smoke_flush_recover() {
        let tmp = TempDir::new().unwrap();

        {
            ...
            let engine = crate::tablet::lsm::TabletEngine::open_with_recovery(
                config,
                lsn_provider,
                ilog,
                Version::new(),
            ...
            let mut batch = new_batch(20);
            batch.put(
                b"tablet_recover_key".to_vec(),
                b"tablet_recover_value".to_vec(),
            );
            engine.write_batch(batch).unwrap();
...

        {
            ...
            assert_eq!(
                get_for_test(&engine, b"tablet_recover_key").await,
                Some(b"tablet_recover_value".to_vec())
            );
        }
    }
